...
void setupWifi() {
  ...
  configTime(0, 3600, ntp_primary, ntp_secondary, ntp_tertiary);
...
void parseConfigUpdate(String payload)
{
  ...
  String on_state = "false";              // from incoming cloud function
  ...
  on_state = payload_doc["on"].as<String>();             // from incoming cloud function
  command = payload_doc["command"].as<String>();
  temp = payload_doc["temperatureSetpointCelsius"];
  cooking_time = payload_doc["timerRemainingSec"];
  recipe = payload_doc["currentCookingMode"].as<String>();
  isStarted = payload_doc["start"].as<String>();
  ...
  if(isStarted == "false" || on_state == "false")
    command = "CANCEL";
