...
bool CloudIoTCoreMqtt::mqttConnect(bool skip) {
  ...
  // Set QoS to 1 (ack) for configuration messages. QoS 1 means that the config message will be received at least once.
...
/*
 * NOTES on how Cloud IoT Core Pub/Sub works
 * There is one events topic per device, and messages published to it
 * are forwarded to the corresponding registry's default telemtry topic.
 * A subfolder can be specified in order to publish to additional topics
 * and in this case the path will be /devices/DEVICE_ID/events/'subfolder'.
 * QoS 0 = delivered at most once. Message may be dropped if a service is unavailable.
 * QoS 1 = delivered at least once. Message will be retried until device acknowledges it. 
 * If a newer message is sent before the old one is received, the older message will be dropped. USE FOR CONFIG AND STATE.
 * QoS 2 = delivered exactly once. Not supported by Cloud IoT Core and will be downgraded to QoS 1.
*/

// only the message data is specified meaning that it will be published to the default
// telemtry topic with a QoS of 0 (no ack). 
bool CloudIoTCoreMqtt::publishTelemetry(String data) {
  return this->mqttClient->publish(device->getEventsTopic(), data);
}

// sends the message to the default events topic with a specified QoS.
...
// Specifies a subfolder (additional topic) to send the message to.
...
// Send the message to a specified subfolder and with a specified QoS.
...
/*
 * NOTES on how Cloud IoT Core Pub/Sub works
 * Like events (telemetry) there is a default topic per device and all messages published to it
 * are forwarded to the corresponding registry's default state topic. No subfolders can be specified,
 * meaning that state updates can only be published to this one topic.
*/

// Send a state update to the cloud, can only go to the default state topic.
...
void CloudIoTCoreMqtt::onConnect() {
  if (logConnect) {
    publishState("ESP32 connected to IoT Core");
